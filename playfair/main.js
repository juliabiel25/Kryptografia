...
function decodePlayfair(text, arr) {
    // decode pairs of characters
    let arrSize = arr.length;
    let decoded = '';
    for (let i = 0; i < text.length-1; i+=2) {
        let loc1 = matrixIndexOf(arr, text[i]);
        let loc2 = matrixIndexOf(arr, text[i+1]);
        let dec1, dec2;
...
function encodePlayfair(text, arr) {
...
function splitRepeating(text, separator) {
    let i = 1;
    let splits = 0;
    while (i < text.length) {
        if (text[i - 1] == text[i]) {
...
function makePairs(text) {
    ...
    for (let i = 0; i < text.length-1; i+=2) {
        if (text[i] == text[i + 1]) {
            console.log("cannot form character pairs without repetition");
            return i+1;
        } else {
...
function onSubmit() {
    ...
    if (warnings.length > 0) {
        ...
        // .......
        let splits;        
        let split = splitRepeating(text, separator);
        if (!isNaN(split)) {
        ...
        const result = encode ? encodePlayfair(text, codingArray) : decodePlayfair(text, codingArray);
